...
export default function MobileNavigation() {
  ...
  // Nút ở Bottom Nav (Tổng quan ở giữa)
  const navItems = [
    { path: '/students', label: 'Học sinh', icon: Users },
    { path: '/attendance', label: 'Điểm danh', icon: QrCode },
    { path: '/', label: 'Tổng quan', icon: Home },
    { path: '/pos', label: 'Học phí', icon: CreditCard },
  ];

  return (
    <div className="lg:hidden">
      {/* Mobile Top Header */}
      <div className="fixed top-0 left-0 right-0 h-20 z-40 bg-white dark:bg-slate-900 flex items-center justify-between px-4 shadow-sm border-b border-slate-100 dark:border-slate-800">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link to="/users" className="w-8 h-8 rounded-full bg-orange-100 text-orange-600 flex items-center justify-center border border-orange-200">
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center">
          ...
          <div className="relative bg-white dark:bg-slate-800 w-full rounded-t-3xl sm:rounded-3xl p-6 shadow-2xl animate-in slide-in-from-bottom-full duration-300">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-lg font-bold text-slate-800 dark:text-white">Menu Mở Rộng</h3>
              <button onClick={() => setShowMoreMenu(false)} className="p-2 bg-slate-100 dark:bg-slate-700 rounded-full text-slate-500 hover:text-slate-800 dark:hover:text-white">
                <X className="w-5 h-5" />
              </button>
            </div>
            
            <div className="space-y-3">
              <Link to="/attendance-logs" onClick={() => setShowMoreMenu(false)} className="flex items-center gap-4 p-4 rounded-2xl bg-slate-50 dark:bg-slate-700/50 text-slate-700 dark:text-slate-200 font-semibold hover:bg-orange-50 hover:text-orange-600 transition-colors">
      ...
      <div className="fixed bottom-0 left-0 right-0 h-16 z-40 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 flex items-center justify-around px-2 pb-safe shadow-[0_-4px_20px_rgba(0,0,0,0.05)] dark:shadow-none">
            ...
            <React.Fragment key={item.path}>
              <Link
                ...
                className={`flex flex-col items-center justify-center w-16 h-full space-y-1 transition-colors ${
                  isActive ? 'text-orange-500' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300'
                }`}
              >
        ...
        <button
          ...
          className={`flex flex-col items-center justify-center w-16 h-full space-y-1 transition-colors ${
            showMoreMenu ? 'text-orange-500' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300'
          }`}
        >
