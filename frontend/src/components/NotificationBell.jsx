  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        ...
        {label && (
          <span className="ml-3 text-[13px] font-semibold">{label}</span>
        )}
      </button>

      {showDropdown && (
        <div
          className={`absolute ${align === "right" ? "right-0" : "left-0 sm:left-full sm:top-0 sm:ml-2 sm:-mt-2"} mt-2 w-80 bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 z-[110] overflow-hidden flex flex-col`}
        >
                ...
                {notifications.map((notification) => (
                  ...
                  >
                    <div className="flex gap-3 items-start">
                      <div
                        className={`mt-0.5 w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${notification.is_read ? "bg-slate-100 text-slate-400 dark:bg-slate-700 dark:text-slate-400" : "bg-cyan-100 text-cyan-600 dark:bg-cyan-900/50 dark:text-cyan-400"}`}
                      >
