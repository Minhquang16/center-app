...
export default function ExamRoomManager({
  ...
  const handleAddRoom = (shiftIndex) => {
    ...
    newShifts[shiftIndex].rooms.push({
      name: `Phòng ${newShifts[shiftIndex].rooms.length + 1}`,
      capacity: 20,
    });
    setShifts(newShifts);
  };
  ...
  const handleAutoAssign = async () => {
    ...
    const unassignedCandidates = candidates.filter(
      (c) => !c.exam_room_id && !c.is_absent,
    );
    if (unassignedCandidates.length === 0) {
      return toast.error(
        "Tất cả thí sinh đã có phòng hoặc không có thí sinh mới.",
      );
    }
    ...
    shifts.forEach((shift) => {
      shift.rooms.forEach((room) => {
        ...
        let occupied = 0;
        if (room.id) {
          occupied = candidates.filter(
            (c) => c.exam_room_id === room.id,
          ).length;
        }
        availableRooms.push({
    ...
    for (let c of shuffled) {
      ...
      while (
        currentRoomIndex < availableRooms.length &&
        availableRooms[currentRoomIndex].capacity <= 0
      ) {
        currentRoomIndex++;
      }
      ...
      if (currentRoomIndex >= availableRooms.length) {
        toast.error(
          `Không đủ phòng. Đã xếp ${assignments.length}/${shuffled.length} thí sinh.`,
        );
        break;
      }
      ...
      <div className="relative bg-white dark:bg-slate-800 rounded-3xl p-6 shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto space-y-4 p-1">
          {shifts.map((shift, sIdx) => (
            <div
              key={sIdx}
              className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-2xl border border-slate-200 dark:border-slate-700"
            >
              ...
              <div className="pl-4 border-l-2 border-cyan-200 dark:border-cyan-900/50 space-y-3">
                <h4 className="text-sm font-bold text-slate-700 dark:text-slate-300 flex items-center justify-between">
                  ...
                  <button
                    onClick={() => handleAddRoom(sIdx)}
                    className="text-cyan-600 text-xs flex items-center gap-1 hover:underline"
                  >
                    ...
                    >
                      ...
                      <div className="w-20 relative">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            const newShifts = [...shifts];
                            newShifts[sIdx].rooms[rIdx].capacity =
                      ...
                      <button
                        onClick={() => {
                          const newShifts = [...shifts];
                          newShifts[sIdx].rooms = newShifts[sIdx].rooms.filter(
                            (_, i) => i !== rIdx,
                          );
                          setShifts(newShifts);
                        }}
          ...
          <button
            ...
            className="w-full py-4 border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-2xl text-slate-500 hover:border-cyan-500 hover:text-cyan-600 transition-colors font-semibold flex items-center justify-center gap-2"
          >
