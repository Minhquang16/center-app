  ...
  return (
    ...
    >
      <div className="flex flex-col space-y-2 p-3 flex-1 overflow-hidden">
        <div
          className={`w-full flex items-center h-20 ${!isCollapsed ? "justify-between px-2" : "justify-center gap-[2px]"}`}
        >
          {!isCollapsed ? (
            ...
            <Link to="/" className="flex items-center justify-center w-8">
              <span className="font-black text-2xl text-orange-500 tracking-tighter">
                S
              </span>
            </Link>
          )}
        ...
        <nav className="w-full space-y-1.5 flex-1 overflow-y-auto overflow-x-hidden py-2 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          {menuItems.map((item) => {
            ...
            if (item.subItems) {
              ...
              const isOpen =
                openSubMenus[item.label] || (!isCollapsed && isActive);
              ...
              return (
                <div key={item.label} className="mx-1">
                  ...
                  >
                    <div
                      className={`flex items-center justify-center transition-all duration-300 flex-shrink-0 ${isCollapsed ? "w-full" : "w-5"}`}
                    >
                      <Icon className="w-5 h-5" />
                    </div>
                    <span
                      className={`text-[13px] whitespace-nowrap overflow-hidden transition-all duration-300 ${isCollapsed ? "max-w-0 opacity-0 ml-0" : "max-w-[200px] opacity-100 ml-3 flex-1"}`}
                    >
                      {item.label}
                    </span>
                    {!isCollapsed && (
            ...
            return (
              ...
              >
                ...
                <span
                  className={`text-[13px] whitespace-nowrap overflow-hidden transition-all duration-300 ${isCollapsed ? "max-w-0 opacity-0 ml-0" : "max-w-[200px] opacity-100 ml-3"}`}
                >
                  {item.label}
                </span>
      ...
      <div className="p-3 space-y-2 flex-shrink-0">
        ...
        >
          <div
            className={`flex items-center justify-center flex-shrink-0 transition-all duration-300 ${isCollapsed ? "w-full" : "w-5"}`}
          >
            {isDark ? (
              <Sun className="w-5 h-5 text-amber-500" />
            ) : (
              <Moon className="w-5 h-5 text-slate-600 dark:text-slate-400 dark:text-slate-400" />
            )}
          </div>
          <span
            className={`text-[13px] font-semibold whitespace-nowrap overflow-hidden transition-all duration-300 ${isCollapsed ? "max-w-0 opacity-0 ml-0" : "max-w-[200px] opacity-100 ml-3"}`}
          >
        ...
        <div
          className={`flex items-center justify-center transition-all duration-300 mx-1 p-0.5 ${isCollapsed ? "w-full" : "w-full justify-start"}`}
        >
          <NotificationBell
            align="left"
            label={!isCollapsed ? "Thông báo" : null}
          />
        </div>

        {/* Profile / Logout section */}
        <div
          className={`mt-2 flex items-center rounded-2xl bg-slate-50 dark:bg-slate-900/50 dark:bg-slate-900/50 dark:bg-slate-700/30 border border-slate-100 dark:border-slate-700/50 dark:border-slate-700/50 dark:border-slate-700 p-2 transition-all duration-300 mx-1 ${
            isCollapsed
        ...
        >
          ...
          >
            ...
            <div
              className={`flex flex-col whitespace-nowrap overflow-hidden transition-all duration-300 ${isCollapsed ? "max-w-0 opacity-0" : "max-w-[150px] opacity-100 ml-1"}`}
            >
