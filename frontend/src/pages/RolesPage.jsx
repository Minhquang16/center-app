                  ...
                  >
                    ...
                    <td className="p-4 text-sm text-slate-600 dark:text-slate-400 whitespace-normal min-w-[300px]">
                      <div className="flex flex-wrap gap-1.5">
                        {role.name === "admin" ? (
                          <span className="bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 px-2.5 py-1 rounded-md text-xs font-medium border border-green-200 dark:border-green-800">
                            Toàn quyền hệ thống
                          </span>
                        ) : (
                          role.permissions.map((p) => (
                            <span
                              key={p.id}
                              className="bg-slate-100 text-slate-700 dark:bg-slate-700 dark:text-slate-300 px-2.5 py-1 rounded-md text-xs border border-slate-200 dark:border-slate-600 whitespace-nowrap"
                            >
                    ...
                    <td className="p-4 pr-6 text-right">
                      <div className="flex justify-end gap-2">
                        <button
                          ...
                          className="p-2 text-slate-400 hover:text-cyan-600 bg-slate-100 dark:bg-slate-800 hover:bg-cyan-50 dark:hover:bg-cyan-900/30 rounded-lg"
                        >
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-slate-800 w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center shrink-0">
              ...
              <button
                onClick={handleCloseModal}
                className="text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 p-1 rounded-lg"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-sm font-semibold text-slate-700 dark:text-slate-300 block mb-1">
                    Tên Vai Trò (Ví dụ: manager, teacher...)
                  </label>
                  <input
                    type="text"
                    value={formData.name}
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                    {permissions.map((perm) => (
                      <label
                        key={perm.id}
                        className="flex items-center gap-2 p-2 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700/50 cursor-pointer border border-transparent hover:border-slate-200 dark:hover:border-slate-600 transition-colors"
                      >
                        <input
                          type="checkbox"
            ...
            <div className="p-4 border-t border-slate-100 dark:border-slate-700 flex justify-end gap-3 shrink-0">
              <button
                onClick={handleCloseModal}
                className="px-5 py-2.5 rounded-xl text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700"
              >
                Hủy
              </button>
              <button
