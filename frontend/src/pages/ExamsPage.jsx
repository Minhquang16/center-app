...
export default function ExamsPage() {
  ...
  const fetchClassesWithStudents = async () => {
    try {
      ...
      toast.error(
        err.response?.data?.message || "Lỗi khi tải danh sách lớp học",
      );
    }
  };
  ...
  const handleAddStudents = async () => {
    ...
    try {
      const res = await api.post(`/exams/${selectedExam.id}/candidates`, {
  ...
  const handleBulkSaveScores = async () => {
    const updates = Object.keys(editingScores).map((id) => ({
      ...
      total_score:
        editingScores[id].total_score === ""
          ? null
          : parseFloat(editingScores[id].total_score),
  ...
  const handleExportExcel = async () => {
    try {
      ...
      if (selectedShift && selectedShift.id !== "unassigned")
        params.append("shift_id", selectedShift.id);
      else if (selectedShift && selectedShift.id === "unassigned")
        params.append("shift_id", "unassigned");

      if (selectedRoom && selectedRoom.id !== "unassigned")
        params.append("room_id", selectedRoom.id);
      else if (selectedRoom && selectedRoom.id === "unassigned")
        params.append("room_id", "unassigned");

      const res = await api.get(
        `/exams/${selectedExam.id}/export?${params.toString()}`,
        { responseType: "blob" },
      );
      ...
      if (selectedShift && selectedShift.name)
        safeName += `_${selectedShift.name.replace(/\s+/g, "_")}`;
      if (selectedRoom && selectedRoom.name)
        safeName += `_${selectedRoom.name.replace(/\s+/g, "_")}`;
  ...
  const handleImportExcel = async () => {
    ...
    try {
      const res = await api.post(
        `/exams/${selectedExam.id}/import-excel`,
        formData,
      );
      toast.success(res.data.message);
      setShowAddStudentsModal(false);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                Ngày thi:{" "}
                {new Date(selectedExam.exam_date).toLocaleDateString("vi-VN")} |
                Trạng thái: {getExamStatus(selectedExam).text}
              </p>
            </div>
          </div>
                  ...
                  <button
                    onClick={async () => {
                      try {
                        ...
                        toast.error(
                          err.response?.data?.message || "Lỗi khi xếp phòng",
                        );
                      }
                    }}
            ...
            <div className="flex items-center gap-3">
              <select
                ...
                onChange={(e) => {
                  ...
                  if (e.target.value === "unassigned") {
                    setSelectedShift({ id: "unassigned" });
                    setSelectedRoom(null);
                    return;
                  }
                  const s = selectedExam.shifts?.find(
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  if (e.target.value === "unassigned") {
                    setSelectedRoom({ id: "unassigned" });
                    return;
                  }
                  const r = selectedShift?.rooms?.find(
                    (x) => x.id == e.target.value,
                  );
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-900/50">
                  ...
                  <th className="p-4 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase text-center">
                    Bỏ thi
                  </th>
                  {selectedExam.scoring_type === "multiple_subjects" &&
                    selectedExam.display_settings?.subjects && (
                      <>
                        {selectedExam.display_settings.subjects.map(
                          (subj, idx) => (
                            <th
                              key={idx}
                              className="p-4 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase"
                            >
                              {subj.name}
                            </th>
                          ),
                ...
                {candidates
                  .filter((c) => {
                    ...
                    if (
                      selectedShift &&
                      selectedShift.id !== "unassigned" &&
                      c.exam_shift_id !== selectedShift.id
                    )
                      return false;
                    if (
                      selectedRoom &&
                      selectedRoom.id === "unassigned" &&
                      c.exam_room_id
                    )
                      return false;
                    if (
                      selectedRoom &&
                      selectedRoom.id !== "unassigned" &&
                      c.exam_room_id !== selectedRoom.id
                    )
                      return false;
                    return true;
                  })
                  .map((c) => (
                    <tr
                      key={c.id}
                      className={`hover:bg-slate-50 dark:hover:bg-slate-750 transition-colors border-b border-slate-100 dark:border-slate-700 last:border-0 ${c.is_scholarship ? "bg-amber-50/50 dark:bg-amber-900/10" : ""}`}
                    >
                      ...
                      <td className="px-4 py-2 font-bold text-slate-800 dark:text-slate-200">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          <span className="text-xs font-semibold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/30 px-1.5 py-0.5 rounded">
                            {c.shift?.name || "Chưa xếp ca"} -{" "}
                            {c.room?.name || "Chưa xếp phòng"}
                          </span>
                        </div>
                        {isAdmin &&
                          selectedExam.status !== "completed" &&
                          selectedExam.shifts?.length > 0 && (
                            <select
                              ...
                              onChange={(e) => {
                                ...
                                const [shiftId, roomId] =
                                  e.target.value.split("-");
                                handleChangeCandidateRoom(
                                  c.id,
                                  shiftId,
                                  roomId,
                                );
                      ...
                      <td className="px-4 py-2 text-center">
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            if (e.target.checked)
                              handleScoreChange(c.id, "note", "Bỏ thi");
                          }}
                        />
                      </td>
                      ...
                      <td className="px-4 py-2">
                        {dynamicRanks[c.id] || c.rank ? (
                          <div className="flex items-center gap-2">
                            <span
                              className={`font-black text-lg ${(dynamicRanks[c.id] || c.rank) <= 3 ? "text-amber-500" : "text-slate-600 dark:text-slate-400"}`}
                            >
                              #{dynamicRanks[c.id] || c.rank}
                            </span>
                            {(dynamicRanks[c.id] || c.rank) <= 3 && (
                      ...
                      <td className="px-4 py-2">
                        <input
                          ...
                          onChange={(e) =>
                            handleScoreChange(c.id, "note", e.target.value)
                          }
                        />
                      </td>
                    </tr>
              ...
              <div className="flex-1 overflow-hidden flex flex-col">
                {addTab === "class" ? (
                  <>
                      ...
                      <div className="flex-1 overflow-y-auto space-y-2 border border-slate-200 dark:border-slate-700 rounded-xl p-2 min-h-[200px]">
                        {classesData
                          ...
                          ?.students?.map((student) => (
                            <label
                              key={student.id}
                              className="flex items-center gap-3 p-3 hover:bg-slate-50 dark:hover:bg-slate-700 rounded-lg cursor-pointer transition-colors border border-transparent hover:border-slate-100 dark:hover:border-slate-600"
                            >
                              <input
                                type="checkbox"
                    ...
                    <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-slate-100 dark:border-slate-700">
                      <button
                        type="button"
                        onClick={() => setShowAddStudentsModal(false)}
                        className="px-5 py-2.5 rounded-xl font-semibold text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-700"
                      >
                        Đóng
                      </button>
                      <button
                        ...
                        className="px-5 py-2.5 rounded-xl font-bold bg-cyan-600 hover:bg-cyan-700 text-white shadow-lg shadow-cyan-600/20 disabled:opacity-50"
                      >
                  ...
                  <>
                    <div className="mb-4">
                      <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">
                        Tải lên file danh sách (.xlsx, .xls, .csv)
                      </label>
                      <div className="mt-2 flex justify-center rounded-xl border-2 border-dashed border-slate-300 dark:border-slate-700 px-6 py-10 hover:border-cyan-500 transition-colors bg-slate-50 dark:bg-slate-800/50">
                        <div className="text-center">
                          ...
                          <p className="text-xs leading-5 text-slate-500 mt-2">
                            Chỉ hỗ trợ file Excel / CSV (Cột đầu là Mã HS / SBD)
                          </p>
                          {excelFile && (
                    ...
                    <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-slate-100 dark:border-slate-700">
                      ...
                      <button
                        ...
                        className="flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold bg-emerald-600 hover:bg-emerald-700 text-white shadow-lg shadow-emerald-600/20 disabled:opacity-50"
                      >
                        {importing && (
        ...
        {exams.map((exam) => (
          ...
          >
            ...
            <div className="flex justify-between items-start">
              ...
              <span
                className={`px-2.5 py-1 text-xs font-bold rounded-full ${getExamStatus(exam).colorClass}`}
              >
            ...
            <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-700 flex justify-between items-center">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="bg-cyan-50 dark:bg-cyan-900/30 text-cyan-600 dark:text-cyan-400 font-semibold text-sm px-4 py-2 rounded-lg hover:bg-cyan-100 dark:hover:bg-cyan-900/50 flex items-center gap-1"
                >
        ...
        {exams.length === 0 && !loading && (
          <div className="col-span-full py-12 text-center border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-2xl">
            ...
            <p className="text-slate-500 dark:text-slate-400 font-medium">
              Chưa có kỳ thi nào. Hãy tạo mới!
            </p>
          </div>
        )}
      </div>
          ...
          <div className="relative bg-white dark:bg-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl w-full max-w-2xl animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleCreateExam} className="space-y-4">
              ...
              {newExam.scoring_type === "multiple_subjects" && (
                <div className="p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl space-y-3">
                  <div className="flex justify-between items-center">
                    <label className="block text-sm font-semibold text-amber-800 dark:text-amber-400">
                      Cấu hình Môn thi & Hệ số
                    </label>
                    <button
                      type="button"
                      onClick={() => {
                        setNewExam({
                          ...
                          display_settings: {
                            ...
                            subjects: [
                              ...newExam.display_settings.subjects,
                              { name: "Môn mới", multiplier: 1 },
                            ],
                          },
                        });
                      }}
                    ...
                    {newExam.display_settings.subjects.map((subj, idx) => (
                      <div key={idx} className="flex gap-2 items-center">
                        ...
                        <button
                          ...
                          onClick={() => {
                            const newSubjects =
                              newExam.display_settings.subjects.filter(
                                (_, i) => i !== idx,
                              );
                            setNewExam({
                              ...newExam,
              ...
              <div className="p-4 bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-800 rounded-xl space-y-3">
                <div className="flex justify-between items-center">
                  <label className="block text-sm font-semibold text-indigo-800 dark:text-indigo-400">
                    Tạo sẵn Ca / Phòng thi
                  </label>
                  <button
                    type="button"
                    onClick={() => {
                      setNewExam({
                        ...
                        shifts: [
                          ...newExam.shifts,
                          {
                            name: `Ca ${newExam.shifts.length + 1}`,
                            start_time: "08:00",
                            end_time: "10:00",
                            room_count: 1,
                            capacity_per_room: 24,
                ...
                <div className="space-y-3">
                  {newExam.shifts.map((shift, idx) => (
                    <div
                      key={idx}
                      className="bg-white dark:bg-slate-900 p-3 rounded-xl border border-indigo-100 dark:border-indigo-700"
                    >
                      <div className="flex gap-2 items-center mb-2">
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            const newShifts = newExam.shifts.filter(
                              (_, i) => i !== idx,
                            );
                            setNewExam({ ...newExam, shifts: newShifts });
                          }}
                      ...
                      <div className="grid grid-cols-4 gap-2">
                        ...
                        <div>
                          ...
                          <input
                            type="number"
                            min="1"
                            value={shift.room_count}
                            onChange={(e) => {
                              const newShifts = [...newExam.shifts];
                              newShifts[idx].room_count =
                        ...
                        <div>
                          ...
                          <input
                            type="number"
                            min="1"
                            value={shift.capacity_per_room}
                            onChange={(e) => {
                              const newShifts = [...newExam.shifts];
                              newShifts[idx].capacity_per_room =
              ...
              <div className="pt-4 flex justify-end gap-3">
                ...
                <button
                  type="submit"
                  className="px-5 py-2.5 rounded-xl font-bold bg-cyan-600 hover:bg-cyan-700 text-white shadow-lg shadow-cyan-600/20"
                >
                  Tạo kỳ thi
                </button>
              </div>
