...
export default function UsersPage({ isEmbedded = false }) {
  ...
  const openEditModal = (user) => {
    ...
    setFormData({
      ...
      email: user.email,
      password: "",
      role: user.role_name || roles[0]?.name || "",
  ...
  return (
    <div
      className={`space-y-6 text-slate-800 dark:text-slate-200 ${isEmbedded ? "" : "pb-12"}`}
    >
      {/* Header */}
      {!isEmbedded && (
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-700/50">
              {loading ? (
                <tr>
                  <td
                    colSpan="5"
                    className="p-8 text-center text-slate-500 dark:text-slate-400 dark:text-slate-400"
                  >
                    <div className="flex flex-col items-center gap-3">
                      <div className="w-8 h-8 border-4 border-cyan-500 border-t-transparent rounded-full animate-spin"></div>
                      <span>Đang tải dữ liệu...</span>
                ...
                <tr>
                  <td
                    colSpan="5"
                    className="p-8 text-center text-slate-500 dark:text-slate-400 dark:text-slate-400 dark:text-slate-400"
                  >
                    Không tìm thấy người dùng nào
                  </td>
                </tr>
                ...
                filteredUsers.map((u, index) => (
                  <tr
                    key={u.id}
                    className="hover:bg-slate-50 dark:hover:bg-slate-700/50 dark:bg-slate-900/50 dark:hover:bg-slate-700/50 dark:bg-slate-900/50 dark:hover:bg-slate-700/30 transition-colors"
                  >
          ...
          <div className="relative bg-white dark:bg-slate-800 w-full max-w-md rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 dark:border-slate-700 dark:border-slate-700 overflow-hidden transform transition-all animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-700/50 dark:border-slate-700/50 dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-900/50 dark:bg-slate-900/50 dark:bg-slate-800/50">
              <h3 className="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2">
                {editingUser ? (
                  <>
                    <Edit className="w-5 h-5 text-amber-500" /> Cập Nhật Thông
                    Tin
                  </>
                ) : (
                  <>
                    <UserPlus className="w-5 h-5 text-cyan-500" /> Thêm Tài
                    Khoản
                  </>
                )}
              </h3>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="space-y-1.5">
                <label className="text-sm font-semibold text-slate-700 dark:text-slate-300 dark:text-slate-300 dark:text-slate-300 flex items-center gap-1.5">
                  <Building className="w-4 h-4" /> Cơ sở{" "}
