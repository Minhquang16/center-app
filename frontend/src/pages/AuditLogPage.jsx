...
export default function AuditLogPage() {
  ...
  const renderDataChange = (log) => {
      ...
      try {
        ...
        return (
          <div className="text-xs space-y-1">
            <div className="text-slate-600 dark:text-slate-400 font-medium bg-slate-50 dark:bg-slate-900/50 px-2 py-1 rounded border border-slate-100 dark:border-slate-700/50">
              Ca học: {newD.shift || "-"}
            </div>
            <div className="text-slate-600 dark:text-slate-400 font-medium bg-slate-50 dark:bg-slate-900/50 px-2 py-1 rounded border border-slate-100 dark:border-slate-700/50">
              Số lượng: {newD.student_count || 0} học sinh
            </div>
          </div>
        );
      } catch {
      ...
      try {
        ...
        return (
          <div className="text-xs space-y-1">
            {d.amount && (
              <div className="text-slate-600 dark:text-slate-400 font-medium bg-slate-50 dark:bg-slate-900/50 px-2 py-1 rounded border border-slate-100 dark:border-slate-700/50">
                Số tiền: {Number(d.amount).toLocaleString("vi-VN")} đ
              </div>
            )}
            {d.method && (
              <div className="text-slate-600 dark:text-slate-400 font-medium bg-slate-50 dark:bg-slate-900/50 px-2 py-1 rounded border border-slate-100 dark:border-slate-700/50">
                HT: {d.method === "transfer" ? "Chuyển khoản" : "Tiền mặt"}
              </div>
            )}
            {d.status && (
              <div className="text-slate-600 dark:text-slate-400 font-medium bg-slate-50 dark:bg-slate-900/50 px-2 py-1 rounded border border-slate-100 dark:border-slate-700/50">
    ...
    try {
      ...
      const changes = [];
      const oldScoreStr =
        typeof oldD.score === "object"
          ? JSON.stringify(oldD.score)
          : oldD.score;
      const newScoreStr =
        typeof newD.score === "object"
          ? JSON.stringify(newD.score)
          : newD.score;

      if (oldScoreStr !== newScoreStr) {
        changes.push(
          `Điểm: ${formatScore(oldD.score)} ➔ ${formatScore(newD.score)}`,
        );
      }

      if (oldD.homework_status !== newD.homework_status)
        changes.push(
          `BTVN: ${oldD.homework_status || "-"} ➔ ${newD.homework_status || "-"}`,
        );
      if (oldD.status !== newD.status)
        changes.push(
          `Trạng thái: ${oldD.status || "-"} ➔ ${newD.status || "-"}`,
        );
      ...
      return (
        <div className="text-xs space-y-1">
          {changes.map((c, i) => (
            <div
              key={i}
              className="text-slate-600 dark:text-slate-400 font-medium bg-slate-50 dark:bg-slate-900/50 px-2 py-1 rounded border border-slate-100 dark:border-slate-700/50"
            >
              {c}
            </div>
          ))}
          ...
          {loading ? (
            ...
            <>
              {logs.slice(0, visibleMobileCount).map((log) => (
                <div
                  key={log.id}
                  className="p-4 space-y-3 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors"
                >
                    ...
                    <div>
                      {log.is_attendance_log ? (
                        <div className="font-bold text-slate-800 dark:text-slate-200 text-sm">
                          Lớp: {log.attendance?.classroom?.name || "N/A"}
                        </div>
                      ) : (
                        <div className="font-bold text-slate-800 dark:text-slate-200 text-sm">
                  ...
                  <div className="text-xs text-slate-700 dark:text-slate-300 space-y-2">
                    {log.is_attendance_log ? (
                      ...
                      <div className="font-medium text-slate-500 dark:text-slate-400">
                        Lớp:{" "}
                        {log.student?.classroom?.name ||
                          log.attendance?.classroom?.name ||
                          "N/A"}{" "}
                        {log.action && log.action.startsWith("INVOICE_")
                          ? "- Thu tiền"
                          : ""}
                      </div>
                    )}
            ...
            <tbody className="divide-y divide-slate-100 text-sm">
              {loading ? (
                <tr>
                  <td
                    colSpan="6"
                    className="p-8 text-center text-slate-500 dark:text-slate-400 dark:text-slate-400 dark:text-slate-400 dark:text-slate-400 font-semibold"
                  >
                    Đang tải nhật ký...
                  </td>
                </tr>
                ...
                logs.map((log) => (
                  <tr
                    key={log.id}
                    className="hover:bg-slate-50 dark:hover:bg-slate-700/50 dark:bg-slate-900/50 dark:hover:bg-slate-700/50 dark:bg-slate-900/50 dark:hover:bg-slate-700/50 dark:bg-slate-900/50 dark:hover:bg-slate-700/50 dark:bg-slate-900/50 transition-colors"
                  >
