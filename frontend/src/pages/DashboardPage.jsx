...
export default function DashboardPage() {
  ...
  useEffect(() => {
    if (selectedGradeModal !== null) {
      ...
      api
        .get("/students", { params: { grade: selectedGradeModal } })
        .then((res) => {
  ...
  const getUniqueTimeSlots = (isWeekend) => {
    ...
    timetableClasses.forEach((c) => {
      (c.schedules || []).forEach((s) => {
        const isDayWeekend =
          parseInt(s.dayOfWeek) === 7 || parseInt(s.dayOfWeek) === 8;
  ...
  const weekdayDays = [
    {
      ...
      badgeClass:
        "bg-rose-100/70 text-rose-900 dark:bg-rose-900/50 dark:text-rose-300",
    },
    {
      label: "Thứ 3",
      val: 3,
      ...
      badgeClass:
        "bg-amber-100/70 text-amber-900 dark:bg-amber-900/50 dark:text-amber-300",
    },
    {
      label: "Thứ 4",
      val: 4,
      ...
      badgeClass:
        "bg-emerald-100/70 text-emerald-900 dark:bg-emerald-900/50 dark:text-emerald-300",
    },
    {
      label: "Thứ 5",
      val: 5,
      ...
      badgeClass:
        "bg-purple-100/70 text-purple-900 dark:bg-purple-900/50 dark:text-purple-300",
    },
    {
      label: "Thứ 6",
      val: 6,
      ...
      badgeClass:
        "bg-blue-100/70 text-blue-900 dark:bg-blue-900/50 dark:text-blue-300",
    },
  ];
  const weekendDays = [
    {
    ...
    },
    {
      ...
      val: 8,
      headerClass:
        "text-rose-800 bg-rose-50 dark:bg-rose-900/30 dark:text-rose-300",
      badgeClass:
        "bg-rose-100/70 text-rose-900 dark:bg-rose-900/50 dark:text-rose-300",
    },
  ];
  ...
  const exportToExcel = () => {
    ...
    if (data.unpaid_list && data.unpaid_list.length > 0) {
      ...
      data.unpaid_list.forEach((s) => {
        ...
        const typeStr = s.class_type
          ? s.class_type.startsWith(" ")
            ? s.class_type
            : ` ${s.class_type}`
          : "";
    ...
    XLSX.writeFile(
      wb,
      `Bao_Cao_Dashboard_T${selectedMonth}_${selectedYear}.xlsx`,
    );
  };
  ...
  const maxRevenue = React.useMemo(
    () =>
      data
        ? Math.max(...(data.revenue_chart?.map((i) => i.revenue) || [1]), 1)
        : 1,
    [data],
  );
  const revPoints = React.useMemo(
    () =>
      data?.revenue_chart?.map((d, i) => {
        ...
        const y =
          chartHeight -
          paddingY -
          (d.revenue / maxRevenue) * (chartHeight - 2 * paddingY);
        return { x, y, val: d.revenue, month: d.month };
      }) || [],
    [data, maxRevenue, chartWidth, chartHeight, paddingX, paddingY],
  );

  const revSmoothPath = React.useMemo(
    () => getSmoothPath(revPoints),
    [revPoints],
  );
  const revAreaPath = React.useMemo(
    () => getAreaPath(revPoints, chartHeight, paddingY),
    [revPoints, chartHeight, paddingY],
  );
  ...
  const maxStudents = React.useMemo(
    () =>
      data
        ? Math.max(
            ...(data.student_movement_chart?.map((i) =>
              Math.max(i.in, i.out),
            ) || [1]),
            1,
          )
  ...
  const inPoints = React.useMemo(
    () =>
      data?.student_movement_chart?.map((d, i) => {
        const x = paddingX + i * ((chartWidth - 2 * paddingX) / 11);
        const y =
          chartHeight -
          paddingY -
          (d.in / maxStudents) * (chartHeight - 2 * paddingY);
        return { x, y, val: d.in, month: d.month };
      }) || [],
    [data, maxStudents, chartWidth, chartHeight, paddingX, paddingY],
  );

  const outPoints = React.useMemo(
    () =>
      data?.student_movement_chart?.map((d, i) => {
        const x = paddingX + i * ((chartWidth - 2 * paddingX) / 11);
        const y =
          chartHeight -
          paddingY -
          (d.out / maxStudents) * (chartHeight - 2 * paddingY);
  ...
  const outSmoothPath = React.useMemo(
    () => getSmoothPath(outPoints),
    [outPoints],
  );
    ...
    <div className="space-y-6 text-slate-800 dark:text-slate-200 dark:text-slate-200 pb-12">
      ...
      <div className="bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm dark:shadow-none dark:shadow-none border border-slate-200 dark:border-slate-700 dark:border-slate-700 flex flex-wrap gap-3 items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-400 mt-0.5">
            Theo dõi sức khỏe tài chính, sĩ số học sinh và tình trạng học phí
            theo thời gian thực
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
        ...
        <div className="bg-white dark:bg-slate-800 p-3 md:p-4 rounded-xl border border-slate-200 dark:border-slate-700 dark:border-slate-700 shadow-sm dark:shadow-none dark:shadow-none space-y-1.5 md:space-y-2">
          ...
          <p className="text-[10px] md:text-[11px] text-slate-400 leading-tight">
            Tích lũy từ Tháng 1 đến Tháng 12
          </p>
        </div>
        ...
        <div className="bg-white dark:bg-slate-800 p-3 md:p-4 rounded-xl border border-slate-200 dark:border-slate-700 dark:border-slate-700 shadow-sm dark:shadow-none dark:shadow-none space-y-1.5 md:space-y-2">
          ...
          <p className="text-lg md:text-2xl font-black text-slate-900 dark:text-white">
            ...
            <span className="text-[10px] md:text-xs font-normal text-slate-500 dark:text-slate-400 dark:text-slate-400">
              học sinh
            </span>
          </p>
          <div className="flex flex-col xl:flex-row xl:items-center gap-1.5 md:gap-3 text-[10px] md:text-[11px] font-semibold pt-0.5">
            <span className="text-emerald-700 dark:text-emerald-400 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/30 dark:bg-emerald-900/30 px-1.5 md:px-2 py-0.5 rounded border border-emerald-200/60 inline-flex items-center gap-1 w-fit">
              <UserPlus className="w-3 h-3" /> +{data?.students_in_month || 0}{" "}
              mới
            </span>
            <span className="text-rose-700 dark:text-rose-400 dark:text-rose-400 bg-rose-50 dark:bg-rose-900/30 dark:bg-rose-900/30 px-1.5 md:px-2 py-0.5 rounded border border-rose-200/60 inline-flex items-center gap-1 w-fit">
              <UserMinus className="w-3 h-3" /> -{data?.students_out_month || 0}{" "}
              nghỉ
            </span>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-800 p-3 md:p-4 rounded-xl border border-slate-200 dark:border-slate-700 dark:border-slate-700 shadow-sm dark:shadow-none dark:shadow-none space-y-1.5 md:space-y-2">
          ...
          <div className="flex flex-col xl:flex-row xl:items-baseline justify-between gap-0.5 md:gap-0">
            <p className="text-lg md:text-2xl font-black text-slate-900 dark:text-white">
              {data?.paid_students_count || 0}{" "}
              <span className="text-[10px] md:text-xs font-normal text-emerald-600">
                Đã nộp
              </span>
            </p>
            <p className="text-base md:text-lg font-bold text-rose-600">
              {data?.unpaid_students_count || 0}{" "}
              <span className="text-[10px] md:text-xs font-normal text-rose-500">
                Nợ HP
              </span>
            </p>
          </div>
          <p className="text-[10px] md:text-[11px] text-slate-400 leading-tight">
            Nợ đọng:{" "}
            <b className="text-slate-800 dark:text-slate-200 dark:text-slate-200">
              {Number(data?.total_unpaid_amount || 0).toLocaleString("vi-VN")} đ
            </b>
          </p>
        </div>
      </div>
        ...
        <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-700/50 dark:border-slate-700/50 pb-2">
          <div className="flex items-center space-x-2 text-slate-800 dark:text-slate-200 dark:text-slate-200">
            ...
            <h3 className="font-bold text-sm">
              Thống Kê Sĩ Số Học Sinh Theo Từng Khối Lớp
            </h3>
          </div>
          <span className="text-[11px] font-semibold text-slate-400 italic">
            💡 Bấm vào từng Khối để xem danh sách học sinh
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 lg:grid-cols-10 gap-2">
          {gradesList.map((g) => {
            ...
            return (
              ...
              >
                <p className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 dark:text-slate-400 group-hover:text-cyan-800 dark:text-cyan-300 dark:text-cyan-300">
                  Khối {g}
                </p>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm dark:shadow-none dark:shadow-none border border-slate-200 dark:border-slate-700 dark:border-slate-700 p-5 space-y-5">
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 dark:border-slate-700/50 dark:border-slate-700/50 pb-3">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-400 mt-0.5">
              Lịch học cố định từ Thứ 2 đến Chủ Nhật phân theo các ca chuẩn
            </p>
          </div>
        ...
        <div className="space-y-2">
          ...
          <div className="md:hidden space-y-3">
            {weekdayDays.map((item) => (
              ...
              >
                <div
                  className={`p-2.5 font-bold text-center border-b border-slate-100 dark:border-slate-700 ${item.headerClass}`}
                >
                  {item.label}
                </div>
                <div className="p-3 space-y-2 text-xs">
                  {weekdaySlots.length > 0 ? (
                    weekdaySlots.map((slot) => (
                      <div
                        key={slot}
                        className="flex justify-between items-center border-b border-slate-50 dark:border-slate-700/50 pb-1.5 last:border-0 last:pb-0"
                      >
                        ...
                        <div className="space-x-1.5 flex flex-wrap justify-end gap-y-1 pl-2 w-full">
                          {weekdayMatrix[item.val][slot].length > 0 ? (
                            weekdayMatrix[item.val][slot].map((cls) => (
                              <span
                                key={cls}
                                className={`px-2 py-0.5 rounded font-bold ${item.badgeClass}`}
                              >
                                {cls}
                              </span>
                            ))
          ...
          <div className="hidden md:block overflow-x-auto border border-slate-200 dark:border-slate-700 rounded-xl">
            <table className="w-full text-left text-xs border-collapse min-w-[650px]">
              <thead className="bg-slate-900 text-white font-bold uppercase text-[11px]">
                <tr>
                  <th className="p-2.5 border-b border-slate-800 w-20 text-center whitespace-nowrap">
                    Thứ
                  </th>
                  {weekdaySlots.length > 0 ? (
                    weekdaySlots.map((slot) => (
                      <th
                        key={slot}
                        className="p-2.5 border-b border-slate-800 text-center text-cyan-300 whitespace-nowrap"
                      >
                        {slot}
                      </th>
                    ))
                  ) : (
                    <th className="p-2.5 border-b border-slate-800 text-center text-slate-400 italic">
              ...
              <tbody className="divide-y divide-slate-100 font-semibold">
                {weekdayDays.map((item) => (
                  <tr
                    key={item.val}
                    className="hover:bg-slate-50 dark:hover:bg-slate-700/50 dark:bg-slate-900/50"
                  >
                    <td
                      className={`p-2.5 text-center font-bold ${item.headerClass}`}
                    >
                      {item.label}
                    </td>
                    {weekdaySlots.length > 0 ? (
                        ...
                        <td key={slot} className="p-2.5 text-center">
                          {weekdayMatrix[item.val][slot].length > 0 ? (
                            weekdayMatrix[item.val][slot].map((cls) => (
                              <span
                                key={cls}
                                className={`px-2 py-0.5 rounded font-bold ml-1 mb-1 inline-block ${item.badgeClass}`}
                              >
                                {cls}
                              </span>
                            ))
                ...
                <div className="p-3 space-y-2 text-xs">
                  {weekendSlots.length > 0 ? (
                    weekendSlots.map((slot) => (
                      <div
                        key={slot}
                        className="flex justify-between items-center border-b border-slate-50 dark:border-slate-700/50 pb-2 last:border-0 last:pb-0"
                      >
                        <span className="text-slate-500 dark:text-slate-400 font-semibold whitespace-nowrap">
                          {slot}
                        </span>
          ...
          <div className="hidden md:block overflow-x-auto border border-slate-200 dark:border-slate-700 rounded-xl">
            <table className="w-full text-left text-xs border-collapse min-w-[750px]">
              <thead className="bg-amber-900 text-amber-100 font-bold uppercase text-[11px]">
                <tr>
                  <th className="p-2.5 border-b border-amber-800 w-24 text-center whitespace-nowrap">
                    Thứ
                  </th>
                  {weekendSlots.length > 0 ? (
                    weekendSlots.map((slot) => (
                      <th
                        key={slot}
                        className="p-2.5 border-b border-amber-800 text-center whitespace-nowrap"
                      >
                        {slot}
                      </th>
                    ))
                  ) : (
                    <th className="p-2.5 border-b border-amber-800 text-center text-amber-200 italic">
                      Chưa có khung giờ nào
                    </th>
                  )}
                </tr>
              ...
              <tbody className="divide-y divide-slate-100 font-semibold text-[11px]">
                {weekendDays.map((item) => (
                  ...
                  >
                    <td
                      className={`p-2.5 text-center font-extrabold ${item.headerClass}`}
                    >
                      {item.label}
                    </td>
                    {weekendSlots.length > 0 ? (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-slate-200 dark:border-slate-700 dark:border-slate-700 shadow-sm dark:shadow-none dark:shadow-none space-y-4 relative">
          <div className="flex justify-between items-center border-b border-slate-100 dark:border-slate-700/50 dark:border-slate-700/50 pb-3">
            <div>
              <h3 className="font-bold text-slate-900 dark:text-white text-sm">
                Biểu Đồ Doanh Thu Đường Cong (Năm {selectedYear})
              </h3>
              <p className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-400">
                Uốn lượn mượt mà + Rê chuột xem chỉ số từng tháng
              </p>
            </div>
            <span className="text-xs font-bold text-cyan-800 dark:text-cyan-300 dark:text-cyan-300 bg-cyan-50 dark:bg-cyan-900/30 dark:bg-cyan-900/30 px-2.5 py-1 rounded border border-cyan-200">
              VNĐ
            </span>
          </div>
          ...
          <div className="relative w-full h-52 pt-2">
            ...
            {hoveredRevIndex !== null && revPoints[hoveredRevIndex] && (
              ...
              >
                <p className="text-[10px] text-cyan-300 uppercase font-semibold">
                  Tháng {revPoints[hoveredRevIndex].month}/{selectedYear}
                </p>
                <p className="text-sm font-black mt-0.5 text-white">
                  {Number(revPoints[hoveredRevIndex].val).toLocaleString(
                    "vi-VN",
                  )}{" "}
                  đ
                </p>
            ...
            >
              ...
              {revAreaPath && (
                <path d={revAreaPath} fill="url(#revGradientSmooth)" />
              )}
        ...
        <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-slate-200 dark:border-slate-700 dark:border-slate-700 shadow-sm dark:shadow-none dark:shadow-none space-y-4 relative">
          <div className="flex justify-between items-center border-b border-slate-100 dark:border-slate-700/50 dark:border-slate-700/50 pb-3">
            <div>
              <h3 className="font-bold text-slate-900 dark:text-white text-sm">
                Biểu Đồ Biến Động Học Sinh Đường Cong (Năm {selectedYear})
              </h3>
              <p className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-400">
                Đường cong Học sinh mới (Vào) vs Nghỉ học (Ra)
              </p>
            </div>
          ...
          <div className="relative w-full h-52 pt-2">
            ...
            {hoveredStudentIndex !== null && inPoints[hoveredStudentIndex] && (
              ...
              >
                <p className="text-[10px] text-cyan-300 uppercase font-semibold">
                  Tháng {inPoints[hoveredStudentIndex].month}/{selectedYear}
                </p>
                <div className="flex items-center gap-3 mt-1 text-xs">
                  <span className="text-emerald-400 font-bold">
                    🟢 Nhập học: +{inPoints[hoveredStudentIndex].val}
                  </span>
                  <span className="text-rose-400 font-bold">
                    🔴 Nghỉ học: -{outPoints[hoveredStudentIndex].val}
                  </span>
                </div>
              </div>
            )}
          ...
          <div className="max-h-80 overflow-y-auto divide-y divide-slate-200 text-xs">
            {!data?.unpaid_list || data.unpaid_list.length === 0 ? (
              <div className="p-6 text-center text-slate-500 dark:text-slate-400 dark:text-slate-400">
                🎉 Tuyệt vời! Tất cả học sinh đã hoàn thành nộp học phí tháng
                này.
              </div>
            ) : (
              data.unpaid_list.slice(0, visibleUnpaidCount).map((s) => (
                ...
                >
                  <div>
                    <p className="font-bold text-slate-900 dark:text-white text-sm group-hover:text-cyan-600 transition-colors">
                      {s.full_name}
                    </p>
                    <p className="text-slate-500 dark:text-slate-400 dark:text-slate-400 text-[11px]">
                      Mã: <b>{s.student_code}</b> | Khối {s.grade}{" "}
                      {s.class_type} | PH: <b>{s.parent_name}</b> (
                      {s.parent_phone})
                    </p>
                  </div>
          ...
          <div className="max-h-80 overflow-y-auto divide-y divide-slate-200 text-xs">
            {!data?.paid_list || data.paid_list.length === 0 ? (
              ...
              data.paid_list.slice(0, visiblePaidCount).map((inv) => (
                <div
                  key={inv.id}
                  className="p-3 hover:bg-slate-50 dark:hover:bg-slate-700/50 dark:bg-slate-900/50 dark:hover:bg-slate-700/50 dark:bg-slate-900/50 flex justify-between items-center"
                >
                  <div>
                    ...
                    <p className="text-slate-500 dark:text-slate-400 dark:text-slate-400 text-[11px]">
                      Mã HD:{" "}
                      <b className="text-cyan-700 dark:text-cyan-400 dark:text-cyan-400">
                        {inv.invoice_code}
                      </b>{" "}
                      | Nộp lúc:{" "}
                      {new Date(
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 w-full max-w-3xl shadow-2xl relative border border-slate-200 dark:border-slate-700 dark:border-slate-700 space-y-4 max-h-[85vh] flex flex-col">
            ...
            <div className="flex justify-between items-center border-b border-slate-200 dark:border-slate-700 dark:border-slate-700 pb-3">
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-400 mt-0.5">
                  Hiển thị <b>{filteredGradeStudents.length}</b> /{" "}
            ...
            <div className="flex flex-wrap items-center gap-1.5 text-xs bg-slate-50 dark:bg-slate-900/50 dark:bg-slate-900/50 p-2.5 rounded-xl border border-slate-200 dark:border-slate-700 dark:border-slate-700/80">
              ...
              {["", "CLC", "CC", "CC1", "CC2", "NC", "NC1", "NC2", "NC3"].map(
                (c) => (
                  <button
                    key={c}
            ...
            <div className="overflow-y-auto flex-1 border border-slate-200 dark:border-slate-700 rounded-xl bg-white dark:bg-slate-800">
              ...
              <div className="md:hidden divide-y divide-slate-100 dark:divide-slate-700/50">
                {loadingGradeStudents ? (
                  ...
                  <div className="p-6 text-center text-slate-400 italic text-xs">
                    Không tìm thấy học sinh phù hợp với bộ lọc Lớp "
                    {modalClassFilter || "Tất cả"}".
                  </div>
                ) : (
                  filteredGradeStudents.map((s) => (
                    <div
                      key={s.id}
                      className="p-3.5 space-y-2 hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors"
                    >
                      ...
                      <div className="bg-slate-50 dark:bg-slate-900/50 p-2.5 rounded-lg text-xs space-y-1.5 border border-slate-100 dark:border-slate-700/80">
                        <div className="flex justify-between">
                          ...
                          <span className="font-semibold text-slate-800 dark:text-slate-200">
                            {s.parent_name || "Chưa cập nhật"}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-slate-500">SĐT:</span>
                          <span className="font-mono text-slate-700 dark:text-slate-300">
                            {s.parent_phone || "Chưa cập nhật"}
                          </span>
                        </div>
                ...
                <tbody className="divide-y divide-slate-100">
                  {loadingGradeStudents ? (
                    ...
                    filteredGradeStudents.map((s) => (
                      <tr
                        key={s.id}
                        className="hover:bg-slate-50 dark:hover:bg-slate-700/50 dark:bg-slate-900/50 dark:hover:bg-slate-700/50 dark:bg-slate-900/50 transition-colors"
                      >
                        <td className="p-3 font-bold font-mono text-cyan-800 dark:text-cyan-300 dark:text-cyan-300">
                          {s.student_code}
                        </td>
                        ...
                        <td className="p-3 font-medium text-slate-800 dark:text-slate-200 dark:text-slate-200">
                          {s.parent_name}
                        </td>
                        <td className="p-3 font-mono text-slate-600 dark:text-slate-400 dark:text-slate-400">
                          {s.parent_phone}
                        </td>
                        <td className="p-3 text-right font-extrabold text-slate-900 dark:text-white">
                          {Number(s.price_per_session || 130000).toLocaleString(
                            "vi-VN",
                          )}{" "}
                          đ
                        </td>
