...
function ClassOverview() {
  ...
  const handleBulkUpdateClass = async () => {
    ...
    try {
      ...
      toast.error(
        "Lỗi chuyển lớp: " + (err.response?.data?.message || err.message),
      );
    }
  };
  ...
  const fetchClasses = async () => {
    try {
      ...
      if (selectedClass) {
        openClassDetail(
          res.data.find((c) => c.id === selectedClass.id) || null,
        );
      }
    } catch (err) {
      toast.error(
  ...
  const openClassDetail = async (c) => {
    ...
    try {
      ...
      toast.error(
        err.response?.data?.message || "Lỗi khi tải danh sách học sinh của lớp",
      );
    } finally {
    ...
    return (
      <div className="space-y-6">
        <button
          ...
          className="text-sm font-semibold text-slate-500 hover:text-cyan-600 flex items-center gap-1"
        >
            ...
            <div>
              ...
              <div className="flex gap-4 text-sm text-slate-500 dark:text-slate-400">
                <span className="flex items-center gap-1">
                  <BookOpen className="w-4 h-4" /> Khối {selectedClass.grade}
                </span>
                <span className="flex items-center gap-1">
            ...
            <div className="flex gap-2">
              {selectedStudents.length > 0 ? (
                <button
                  ...
                  className="bg-cyan-600 hover:bg-cyan-700 text-white px-4 py-2 rounded-xl text-sm font-bold shadow-md shadow-cyan-500/20 transition-all flex items-center gap-2"
                >
                  Chuyển lớp ({selectedStudents.length})
                </button>
              ) : null}
              <button
                onClick={(e) => openEditModal(selectedClass, e)}
                className="bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 p-2 rounded-xl transition-colors"
              >
                  ...
                  <>
                    {selectedClass.students?.map((student) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 font-mono font-bold text-cyan-600">
                        ...
                        <td className="px-4 py-3 text-sm font-bold text-center text-cyan-600 dark:text-cyan-400">
                          {student.attended_this_month || 0} buổi
                        </td>
                      </tr>
                    ))}
                    {(!selectedClass.students ||
      ...
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        <div className="relative bg-white dark:bg-slate-800 rounded-3xl p-6 shadow-2xl w-full max-w-md">
          ...
          <div className="mb-4 text-sm text-slate-600 dark:text-slate-400">
            Bạn đang chọn <b>{selectedStudents.length}</b> học sinh. Vui lòng
            chọn lớp mới để chuyển các học sinh này sang.
          </div>
          <div className="mb-6">
            ...
            >
              <option value="">
                -- Xóa khỏi lớp hiện tại (Không gán lớp) --
              </option>
              {classes.map((c) => (
                <option key={c.id} value={c.class_code}>
                  {c.name} ({c.class_code}) - Khối {c.grade}
                </option>
              ))}
            </select>
          </div>
          <div className="flex gap-3 justify-end">
            <button
              onClick={() => setShowBulkEditModal(false)}
              className="px-5 py-2.5 rounded-xl font-bold text-slate-600 bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 dark:text-slate-300 transition-colors"
            >
              Hủy
            </button>
            <button
              onClick={handleBulkUpdateClass}
              className="px-5 py-2.5 rounded-xl font-bold bg-cyan-600 hover:bg-cyan-700 text-white shadow-md shadow-cyan-500/20 transition-all"
            >
              Xác nhận
            </button>
          </div>
          ...
          <form onSubmit={handleSaveClass} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  required
                  type="text"
                  className="w-full p-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-cyan-500"
                  value={editForm.name}
                  onChange={(e) =>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setEditForm({ ...editForm, class_code: e.target.value })
                  }
                  placeholder="VD: T10A1"
                />
              </div>
            </div>
            <div>
            ...
            <div className="p-4 bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-800 rounded-xl space-y-3 mt-4">
              <div className="flex justify-between items-center">
                <label className="block text-sm font-bold text-indigo-800 dark:text-indigo-400">
                  Thời khóa biểu / Ca học
                </label>
                <button
                  type="button"
                  onClick={addSchedule}
                  className="text-xs bg-indigo-200 dark:bg-indigo-800 text-indigo-800 dark:text-indigo-200 px-3 py-1.5 rounded-lg font-bold hover:bg-indigo-300 transition-colors"
                >
                  + Thêm lịch học
                </button>
              </div>
              ...
              <div className="space-y-2">
                {editForm.schedules.map((s, idx) => (
                  <div
                    key={idx}
                    className="flex gap-2 items-center bg-white dark:bg-slate-900 p-2 rounded-xl border border-indigo-100 dark:border-indigo-700"
                  >
                    <select
                      ...
                      value={s.dayOfWeek}
                      onChange={(e) =>
                        updateSchedule(idx, "dayOfWeek", e.target.value)
                      }
                    >
                    ...
                    <input
                      type="time"
                      className="p-2 text-sm bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg outline-none"
                      value={s.startTime}
                      onChange={(e) =>
                ...
                {editForm.schedules.length === 0 && (
                  <div className="text-xs text-slate-500 italic p-2 text-center">
                    Chưa có lịch học nào. Nhấn Thêm lịch học để tạo Ca học.
                  </div>
                )}
              </div>
            </div>
      ...
      <div className="relative overflow-hidden bg-blue-600 bg-gradient-to-r from-blue-700 to-blue-500 rounded-3xl p-6 text-white shadow-lg shadow-blue-500/20">
        ...
        <div className="relative z-10">
          <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
            <Calendar className="w-6 h-6" /> Lớp học hôm nay
          </h2>
              ...
              {todayClasses.map((c) => (
                ...
                >
                  ...
                  <div className="mt-3 flex items-center justify-between text-sm">
                    <span className="flex items-center gap-1">
                      <Users className="w-4 h-4" /> {c.student_count || 0} HS
                    </span>
                    <span className="flex items-center gap-1 font-semibold">
                      {
                        c.schedules?.find(
                          (s) =>
                            parseInt(s.dayOfWeek) ===
                            (new Date().getDay() === 0
                              ? 8
                              : new Date().getDay() + 1),
                        )?.startTime
      ...
      <div>
        <div className="flex justify-between items-center mb-4">
          ...
          <button
            ...
            className="bg-cyan-600 hover:bg-cyan-700 text-white px-4 py-2 rounded-xl font-bold transition-all shadow-lg shadow-cyan-600/30 flex items-center gap-2 text-sm"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {classes.map((c) => (
            <div
              ...
              className="bg-white dark:bg-slate-800 rounded-2xl p-5 border border-slate-200 dark:border-slate-700 shadow-sm cursor-pointer hover:shadow-md hover:border-cyan-500 transition-all group relative"
            >
              <div className="absolute top-4 right-4 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                <button
                  ...
                  className="p-1.5 bg-red-50 hover:bg-red-100 dark:bg-red-900/30 dark:hover:bg-red-900/50 rounded-lg text-red-500 transition-colors"
                >
                  <Trash2 className="w-4 h-4" />
                </button>
              </div>
              ...
              <div className="flex justify-between items-start mb-3 pr-16">
                <h3 className="font-bold text-slate-800 dark:text-slate-200 group-hover:text-cyan-600 transition-colors text-lg truncate">
                  {c.name}
                </h3>
              </div>
              <div className="space-y-2">
                <span className="bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 text-xs px-2 py-1 rounded font-mono font-semibold">
                  {c.class_code}
                </span>
                <div className="flex items-center text-sm text-slate-500 dark:text-slate-400 gap-2 mt-2">
                  <Users className="w-4 h-4" /> Sĩ số: {c.student_count || 0}{" "}
                  học sinh
                </div>
                {c.schedules && c.schedules.length > 0 ? (
                  <div className="flex flex-wrap gap-1 mt-2">
                    {c.schedules.map((s, idx) => (
                      <span
                        key={idx}
                        className="bg-cyan-50 dark:bg-cyan-900/30 text-cyan-700 dark:text-cyan-400 text-xs px-2 py-1 rounded-md border border-cyan-100 dark:border-cyan-800"
                      >
