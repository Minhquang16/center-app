...
export default function LookupPage() {
  ...
  const handleLookup = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await api.post("/exam-lookup", {
      ...
      if (
        ...
        res.data.results[0].is_scholarship
      ) {
        // Simple confetti effect triggers by adding class or you can use a library
        toast.success("🎉 Chúc mừng bạn đã đạt Học Bổng!", {
          duration: 5000,
          position: "top-center",
        });
      }
      ...
      toast.error(
        err.response?.data?.message || "Không tìm thấy kết quả phù hợp",
      );
    } finally {
      setLoading(false);
    }
        ...
        {!result ? (
          ...
          <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl p-8 rounded-[2rem] shadow-2xl border border-slate-200 dark:border-slate-700 animate-in fade-in zoom-in-95 duration-500">
            <p className="text-center text-slate-500 dark:text-slate-400 mb-8 font-medium">
              Vui lòng nhập Mã học sinh và Số điện thoại phụ huynh để xem kết
              quả kỳ thi mới nhất.
            </p>
              ...
              <div className="bg-gradient-to-br from-cyan-600 to-blue-700 p-8 text-white relative overflow-hidden">
                ...
                <div className="relative z-10">
                  ...
                  <p className="opacity-90 font-medium mt-1 text-cyan-100 flex items-center gap-4">
                    <span>
                      Mã HS:{" "}
                      <b className="font-mono">{result.student.student_code}</b>
                    </span>
                    <span>
                      Khối: <b>{result.student.grade}</b>
                    </span>
                  </p>
                </div>
              </div>
                ...
                {result.results.length === 0 ? (
                  <div className="text-center py-10 text-slate-500">
                    ...
                    <p className="font-medium text-lg">
                      Chưa có kết quả kỳ thi nào được công bố.
                    </p>
                  </div>
                ) : (
                  result.results.map((candidate, idx) => (
                    <div key={idx} className="relative">
                        ...
                        <div className="mb-6 p-4 rounded-2xl bg-gradient-to-r from-amber-500 to-orange-500 text-white flex items-center gap-4 shadow-lg shadow-amber-500/30 animate-pulse">
                          ...
                          <div>
                            <h4 className="font-black text-lg">
                              XUẤT SẮC - ĐẠT HỌC BỔNG!
                            </h4>
                            <p className="text-sm font-medium opacity-90">
                              Học sinh nằm trong Top 3 xuất sắc nhất kỳ thi.
                              Chúc mừng gia đình!
                            </p>
                          </div>
                        </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 bg-slate-50 dark:bg-slate-900/50 p-6 rounded-3xl border border-slate-100 dark:border-slate-700">
                        {candidate.scores &&
                          candidate.scores.toan !== undefined && (
                            <div className="col-span-1">
                              {renderScoreBar(
                                "Môn Toán",
                                candidate.scores.toan,
                              )}
                              {renderScoreBar("Môn Văn", candidate.scores.van)}
                              {renderScoreBar(
                                "Môn Tiếng Anh",
                                candidate.scores.anh,
                              )}
                            </div>
                          )}
                        <div
                          className={`col-span-1 flex flex-col justify-center items-center ${candidate.scores ? "border-l border-slate-200 dark:border-slate-700 pl-6" : "col-span-full"}`}
                        >
                          ...
                          <span className="text-5xl font-black text-slate-800 dark:text-white mt-2">
                            {candidate.total_score}
                          </span>
                          {candidate.note && (
                            <p className="mt-4 text-sm text-center text-slate-500 italic px-4 py-2 bg-slate-100 dark:bg-slate-800 rounded-lg">
                              "{candidate.note}"
                            </p>
                          )}
                        </div>
